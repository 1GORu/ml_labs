...
**Вывод**: даже простая линейная регрессия может оказаться эффективным инструментом для прогнозирования временных рядов, если в данных присутствуют чёткие тренд и сезонность. В случае небольшого объёма наблюдений сложные методы вроде ARIMA или нейросетей часто избыточны - "старая добрая" регрессия не только справляется с задачей, но и даёт интерпретируемое, устойчивое и легко поддерживаемое решение.
